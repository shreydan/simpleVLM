...
class GroupedQueryAttention(nn.Module):
    def __init__(self, config):
        ...
        self.prefix_length = config.num_image_tokens
    ...
    def make_mask(self):
        causal_mask = torch.triu(torch.ones(self.max_position_embeddings,self.max_position_embeddings),diagonal=1)
        # causal_mask[:self.prefix_length, :self.prefix_length] = 0
        return causal_mask
    ...
    def get_attention_mask(self, cm, pm):
        return ~(pm[:,None,None,:] * ~cm)
    ...
    def forward(self, x, attention_mask):
        ...
        mask = self.get_attention_mask(mask, attention_mask)
...
class LLaMA(nn.Module):
    ...
    def disable_kv_cache(self):
        for layer in self.layers:
            layer.self_attn.disable_kv_cache()
